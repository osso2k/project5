...
export const signup = async (req: Request<{},{},User> ,res:Response) =>{
    ...
    try {
        ...
        const user = await pool.query("INSERT INTO users (username , hashedPassword) VALUES ($1,$2) RETURNING id",[username, hashedPassword])
        const token = //gen token wioth id
        res.status(202).json({messsage:"User Created Successfully!",user:user.rows[0].id /*,token*/})
    } catch (error) {
...
export const login = async (req:Request<{},{},User>,res:Response)=>{
    try {
        ...
        const isValid = await bcrypt.compare(password, user.rows[0].hashedPassword)

        if (!isValid){
            return res.status(401).json({message:"Invalid credentials"})
        }
        //gen token
        res.json({message:"Succeessfully logged in!", user:user.rows[0].id /* , token */ })
    } catch (error) {
